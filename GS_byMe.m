...
error = [error; ee];
error1 = [error1; ee1];
uniformity = [uniformity; 1-((max(Iachieved(mask))-min(Iachieved(mask)))/(max(Iachieved(mask))+min(Iachieved(mask))))];
efficiency = [efficiency; sum(Iachieved(mask))/sum(Iachieved(:))];
